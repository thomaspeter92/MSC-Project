...
class UserDb extends Db {
  ...
  public getUserProfile = async (user_id: number) => {
  ...
  public getListOfUsers = async (user_ids: number[]) => {
  ...
  public createUser = async (data: any) => {
    let sql = `INSERT INTO "User" (email, first_name, last_name, age, sex, orientation, verified, complete, password) VALUES ($1, $2, $3, $4, $5, $6, false, false, $7) RETURNING id;`;
    let result = await this.query(sql, [
      data.email,
      data.first_name,
      data.last_name,
  ...
  public createUserProfile = async (data: any) => {
  ...
  public updateEssays = async (data: any) => {
  ...
  public getUnfinishedProfile = async (id: number) => {
    let sql = `SELECT id
    FROM "Profile"
    WHERE user_id = $1
  ...
  public updateUserComplete = async (id: number) => {
