...
const SelectInput = ({ options, label, defaultValue, name, onChange, error }: Props) => {
  ...
  return (
    <div className="w-full">
      {label ?
        <p className="text-rose-500 text-sm font-semibold block mb-2">{label}</p>
        : null}
      <Listbox defaultValue={defaultValue} onChange={handleChange} name={name}>
        <div className={cn("relative w-full rounded", error ? 'border border-red-500' : '')}>
          ...
          <Listbox.Options className="absolute top-[95%] left-0 w-full z-10 rounded-b overflow-hidden">
            {options?.map(d => (
              <Listbox.Option value={d} key={d.value} className={"bg-rose-100 p-3 text-rose-500 capitalize hover:bg-rose-200 cursor-pointer focus:border-1 border-black"}>
                {d.name}
              </Listbox.Option>
            ))}
          </Listbox.Options>
