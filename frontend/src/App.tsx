...
function App() {
  ...
  const signIn = async () => {
    let res = await signInWithEmailAndPassword(auth,'tomas@test.com', 'password123')
    console.log(res)

    let token = await res.user.getIdToken()

    let r = await fetch('http://127.0.0.1:8080/api/v1/user/signin', {method: 'POST', headers: {
  ...
  return (
   <div onClick={signIn}>Application</div>
  )
}
